...
class DisplayUtils:
    ...
    def draw_box_on_image(self, image, categories, ann, color):
        x, y, w, h  = ann["bbox"]
        b,g,r=color
        image = cv2.rectangle(image, (x, y), (x + w, y + h), (b-100,g-100,r-100), self.box_width)
        xyxy1=ann["rotated"][:2]
        xyxy2=ann["rotated"][2:4]
        xyxy3=ann["rotated"][4:6]
        xyxy4=ann["rotated"][6:8]
        
        cv2.circle(image, xyxy1, 3, color, -1)
        cv2.circle(image, xyxy2, 3, color, -1)
        cv2.circle(image, xyxy3, 3, color, -1)
        cv2.circle(image, xyxy4, 3, color, -1)
        line1=np.int0([xyxy1,xyxy2])
        line2=np.int0([xyxy2,xyxy3])
        line3=np.int0([xyxy3,xyxy4])
        line4=np.int0([xyxy4,xyxy1])
        cv2.polylines(image, [line1],isClosed=True, color=color, thickness=2)
        cv2.polylines(image, [line2], isClosed=True, color=color,thickness=2)
        cv2.polylines(image, [line3],isClosed=True, color=color, thickness=2)
        cv2.polylines(image, [line4], isClosed=True, color=color,thickness=2)
        try:
            categories[ann["category_id"]]
            text = '{} {}'.format(ann["category_id"],categories[ann["category_id"]])
